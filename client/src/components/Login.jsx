  ...
  return (
    <div onClick={()=> setShowUserLogin(false)} className="fixed top-0 bottom-0 left-0 right-0 z-30 flex items-center justify-center text-sm text-gray-600 bg-black/50">
        ...
        <p className="text-2xl font-medium m-auto">
           <span class="text-primary">User</span> {state === "login" ? "Login" : "Register"}
        </p>
        {state === "register" && (
    ...
    {state === "register" ? (
         <p>
            Already have an account? <span onClick={() => setState("login")} class="text-primary cursor-pointer">click here</span>
         </p>
        ) : (
        <p>
            Create an account? <span onClick={() => setState("register")} class="text-primary cursor-pointer">click here</span>
         </p>
        )
    }
